...
export default function StopGame() {
  ...
  return <div className="min-h-screen bg-[#17142B] text-white [background-image:radial-gradient(circle_at_85%_10%,rgba(244,63,94,.18),transparent_30%),radial-gradient(circle_at_10%_80%,rgba(190,24,93,.16),transparent_34%)]"><MobileNav/><div className="flex justify-center">
    <GameIdentityLayout
    ...
    >
      ...
      {room.status === "voting" && <section className="flex flex-1 flex-col py-5">
        ...
        <div className="relative overflow-hidden rounded-[2rem] border-2 border-[#F27052]/35 bg-[#292052] p-4 shadow-[0_10px_0_#503FBF,0_24px_70px_rgba(244,63,94,.2)] sm:p-6">
          ...
          <header className="relative flex items-center justify-between gap-3 border-b border-[#EBB3F2]/20 pb-5">
            ...
            <div className="absolute left-1/2 top-[-8px] -translate-x-1/2"><div className="grid h-24 w-24 place-items-center bg-gradient-to-b from-[#F27052] to-[#503FBF] drop-shadow-[0_8px_0_#503FBF] [clip-path:polygon(25%_3%,75%_3%,100%_50%,75%_97%,25%_97%,0_50%)]"><div className="grid h-[78px] w-[78px] place-items-center border-4 border-white/90 [clip-path:inherit]"><span className="text-5xl font-black text-white">{room.letter}</span></div></div><small className="absolute left-1/2 top-2 -translate-x-1/2 text-[8px] font-black uppercase tracking-widest text-white">Letra</small></div>
          ...
          <div className="relative mt-5 flex items-center gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-full border-2 border-[#EBB3F2]/40 bg-[#503FBF]/40"><Clock3 className="h-5 w-5 text-[#EBB3F2]"/></div><div className="h-4 flex-1 overflow-hidden rounded-full border-2 border-[#503FBF] bg-[#503FBF]"><div className="h-full rounded-full bg-gradient-to-r from-[#F27052] to-[#EBB3F2] transition-all duration-100" style={{ width: `${room.votingComplete ? 100 : Math.max(0, Math.min(100, (((room.voteEndsAt || now) - now) / 20_000) * 100))}%` }}/></div></div>
        </div>

        {!room.votingComplete && <button onClick={() => act("vote-ready", { playerId: me.uid, categoryIndex: voteCategory })} disabled={busy || Boolean(room.voteReady[me.uid])} className="mx-auto mt-6 h-16 w-full max-w-md rounded-2xl border-2 border-[#EBB3F2] border-b-[6px] border-b-[#503FBF] bg-gradient-to-r from-[#F27052] to-[#6650F2] text-lg font-black text-white shadow-[0_12px_35px_rgba(244,63,94,.25)] transition-all hover:-translate-y-0.5 hover:brightness-110 active:translate-y-1 active:border-b-2 disabled:border-[#503FBF] disabled:from-[#79D9AC] disabled:to-[#79D9AC] disabled:text-[#503FBF]"><CheckCircle2 className="mr-2 inline h-6 w-6"/>{room.voteReady[me.uid] ? "PRONTO — AGUARDANDO OS OUTROS" : "PRONTO, ESSA É MINHA DECISÃO"}</button>}
      ...
      {room.status === "results" && <section className="flex flex-1 flex-col items-center justify-center py-6 text-center">
        ...
        {isHost ? <button onClick={() => act("lobby", { playerId: me.uid })} className="mt-7 h-16 w-full max-w-2xl rounded-2xl border-2 border-[#EBB3F2] border-b-[6px] border-b-[#503FBF] bg-gradient-to-r from-[#F27052] to-[#6650F2] text-lg font-black"><RotateCcw className="mr-2 inline h-5 w-5"/>NOVA PARTIDA</button> : <p className="mt-7 w-full max-w-2xl rounded-xl border border-[#F27052]/20 bg-[#503FBF]/20 py-4 font-bold text-[#EBB3F2]/50">Aguardando o capitão...</p>}
      </section>}
