...
export default function StopGame() {
  ...
  return <div className="min-h-screen bg-[#1a1e2a] text-white"><MobileNav/><div className="flex justify-center">
    <GameIdentityLayout
      ...
      detail={player => <p className={cn("mt-1 text-[9px] font-black uppercase", room.status === "voting" && room.voteReady[player.uid] ? "text-emerald-300" : "text-violet-300")}>{room.status === "results" ? `${player.score} pontos` : room.status === "voting" ? room.voteReady[player.uid] ? "✓ Decisão confirmada" : "Validando respostas" : player.finished ? "Cartela completa" : room.status === "waiting" ? "Pronto" : "Respondendo"}</p>}
    >
      ...
      {room.status === "voting" && <section className="flex flex-1 flex-col py-5">
        ...
        <div className="relative overflow-hidden rounded-[2rem] border-2 border-violet-400/35 bg-[#25265f] p-4 shadow-[0_10px_0_#101330,0_24px_70px_rgba(76,29,149,.22)] sm:p-6">
          ...
          <header className="relative flex items-center justify-between gap-3 border-b border-violet-300/20 pb-5">
            <div className="rounded-xl border border-violet-300/30 bg-[#171845]/80 px-4 py-2 text-left"><small className="block text-[9px] font-black uppercase tracking-[.18em] text-violet-200">Categorias</small><strong className="text-xl">{voteCategory + 1}<span className="text-sm text-violet-300">/{categories.length}</span></strong></div>
            <div className="absolute left-1/2 top-[-8px] -translate-x-1/2"><div className="grid h-24 w-24 place-items-center bg-[#11183b] drop-shadow-[0_8px_0_#090d25] [clip-path:polygon(25%_3%,75%_3%,100%_50%,75%_97%,25%_97%,0_50%)]"><div className="grid h-[78px] w-[78px] place-items-center bg-cyan-400 [clip-path:inherit]"><span className="text-5xl font-black text-[#101439]">{room.letter}</span></div></div><small className="absolute left-1/2 top-2 -translate-x-1/2 text-[8px] font-black uppercase tracking-widest text-white">Letra</small></div>
            <div className="flex items-center gap-2 rounded-xl border border-violet-300/30 bg-[#171845]/80 px-3 py-2"><Clock3 className="h-6 w-6 text-cyan-300"/><strong className="font-mono text-lg">{room.votingComplete ? "OK" : formatTime(Math.max(0, (room.voteEndsAt || now) - now))}</strong></div>
          </header>

          <div className="relative mt-10 min-h-[390px] rounded-3xl border border-violet-300/20 bg-[#161747]/85 p-5 text-center shadow-inner sm:p-8">
            <p className="text-[10px] font-black uppercase tracking-[.22em] text-cyan-300">Tema da validação</p><h1 className="mt-1 text-4xl font-black">{categories[voteCategory]}</h1><p className="mt-2 text-sm font-bold text-violet-200">{Object.keys(room.voteReady).length}/{room.players.length} jogadores prontos</p>
          ...
          <div className="relative mt-5 flex items-center gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-full border-2 border-violet-300/40 bg-[#14163d]"><Clock3 className="h-5 w-5 text-cyan-300"/></div><div className="h-4 flex-1 overflow-hidden rounded-full border-2 border-[#101334] bg-[#101334]"><div className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-emerald-300 transition-all duration-100" style={{ width: `${room.votingComplete ? 100 : Math.max(0, Math.min(100, (((room.voteEndsAt || now) - now) / 20_000) * 100))}%` }}/></div></div>
        </div>

        {!room.votingComplete && <button onClick={() => act("vote-ready", { playerId: me.uid, categoryIndex: voteCategory })} disabled={busy || Boolean(room.voteReady[me.uid])} className="mx-auto mt-6 h-16 w-full max-w-md rounded-2xl border-2 border-amber-200 border-b-[6px] border-b-amber-700 bg-amber-400 text-lg font-black text-amber-950 shadow-[0_12px_35px_rgba(251,191,36,.2)] transition-all hover:-translate-y-0.5 hover:bg-amber-300 active:translate-y-1 active:border-b-2 disabled:border-emerald-700 disabled:bg-emerald-400 disabled:text-emerald-950"><CheckCircle2 className="mr-2 inline h-6 w-6"/>{room.voteReady[me.uid] ? "PRONTO — AGUARDANDO OS OUTROS" : "PRONTO, ESSA É MINHA DECISÃO"}</button>}
        <div className="mt-6 flex justify-center gap-1.5">{categories.map((_, index) => <span key={index} className={cn("h-2.5 rounded-full transition-all", index < voteCategory || room.votingComplete ? "w-2.5 bg-emerald-400" : index === voteCategory ? "w-8 bg-violet-400" : "w-2.5 bg-slate-700")}/>)}</div>
        {room.votingComplete && isHost ? <button onClick={() => act("finish", { playerId: me.uid })} className="mt-6 h-16 rounded-2xl border-b-4 border-violet-800 bg-violet-500 text-lg font-black">VER RESULTADO DA RODADA</button> : room.votingComplete ? <p className="tj-inset mt-6 py-4 text-center font-bold text-slate-400">Aguardando o capitão mostrar o resultado.</p> : <p className="mt-4 text-center text-xs font-bold text-slate-500">A categoria avança quando todos apertarem pronto ou quando o tempo acabar.</p>}
      </section>}
