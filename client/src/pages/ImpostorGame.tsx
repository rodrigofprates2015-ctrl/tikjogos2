          ...
          <div className="relative h-1 bg-[#1a1c2e] rounded-full mb-5 mx-2">
            <div
              className={cn(
                ...
                selectedGame === 'stop' && "left-[80%] bg-[#6650F2]"
              )}
            />
          </div>
    ...
    <div className="fixed inset-0 z-[40] flex h-[100dvh] flex-col overflow-hidden bg-[#080f20] px-2 pb-[max(.5rem,env(safe-area-inset-bottom))] pt-[max(.5rem,env(safe-area-inset-top))] lg:hidden">
      ...
      <div className="tj-inset mt-2 grid shrink-0 grid-cols-5 gap-1.5 p-1.5">
        {room.players.slice(0, 10).map((player, index) => {
          ...
          return <button type="button" key={player.uid} onClick={() => canSelectVote && setSelectedVote(player.uid)} disabled={!canSelectVote} className={cn("tj-player-card relative flex min-w-0 flex-col items-center px-1 py-1.5", isCurrentUser && "is-current", canSelectVote && "active:scale-95", currentStage === 'VOTING' && selectedVote === player.uid && "is-vote-target", isResultImpostor && !isCurrentUser && "is-danger")}>
