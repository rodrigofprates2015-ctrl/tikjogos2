  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(141,81,251,.16),transparent_34%),radial-gradient(circle_at_bottom_right,rgba(44,126,252,.10),transparent_32%),#1A1E2A] text-white">
    ...
    <div className="relative z-10 mx-auto grid w-full max-w-[1480px] grid-cols-1 items-stretch gap-4 px-2 py-3 sm:px-5 lg:grid-cols-[350px_minmax(0,1fr)] lg:gap-5 lg:py-8">
      ...
      <section className="tj-surface tj-surface--stage order-1 flex min-h-[620px] min-w-0 flex-col overflow-hidden p-3 sm:p-5 lg:order-2 lg:h-[calc(100vh-4rem)] lg:min-h-[620px] lg:p-6">
        ...
        <div className="mt-3 origin-top scale-90"><DigitalLogo compact/></div>
        <main className="mx-auto flex min-h-0 w-full max-w-4xl flex-1 flex-col justify-center space-y-4 py-3">
      {room.status === "waiting" && <>
...
function ClassicResults({ room, bestDifference, busy, isHost, act }: { room: Room; bestDifference?: number; busy: boolean; isHost: boolean; act: (endpoint: string, extra?: Record<string, unknown>) => Promise<void> }) {
  return <><section className="text-center"><Trophy className="mx-auto h-9 w-9 text-yellow-400"/><span className="mt-1.5 block text-[10px] font-black uppercase tracking-[.2em] text-violet-300">Resultado da rodada {room.round}</span><h1 className="mt-1.5 text-2xl font-black">Alvo: {formatTime(room.targetMs || 0)}</h1></section><section className="tj-inset grid min-h-0 gap-2 p-3 sm:grid-cols-2">{room.attempts.map((attempt, index) => <article key={attempt.playerId} className={`tj-player-card flex min-w-0 items-center gap-3 px-3 py-2.5 ${attempt.differenceMs === bestDifference ? "is-selected" : ""}`}><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg text-sm font-black ${index === 0 ? "bg-violet-500 text-white" : "bg-slate-700"}`}>{index + 1}</span><div className="min-w-0 flex-1"><strong className="block truncate text-sm">{attempt.playerName}</strong><span className="block truncate text-xs text-slate-400">Diferença: {formatTime(attempt.differenceMs)}</span></div><strong className="shrink-0 font-mono text-sm text-violet-300">{formatTime(attempt.elapsedMs)}</strong></article>)}</section>{isHost ? <div className="grid gap-2 sm:grid-cols-3"><Button onClick={() => act("repeat")} disabled={busy} variant="gameSecondary" size="game"><Repeat2/> REPETIR</Button><Button onClick={() => act("next")} disabled={busy} variant="gamePrimary" size="game"><RotateCcw/> NOVO TEMPO</Button><Button onClick={() => act("lobby")} disabled={busy} variant="gameSecondary" size="game"><Users/> LOBBY</Button></div> : <p className="text-center text-sm font-bold text-slate-400">Aguardando o líder...</p>}</>;
}
