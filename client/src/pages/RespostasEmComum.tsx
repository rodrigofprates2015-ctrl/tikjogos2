      ...
      <div className="bg-[#242642] rounded-[2rem] p-6 md:p-8 shadow-2xl border-2 border-[#2f3252] w-[90%] max-w-lg z-10">
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-bold text-gray-300">Jogadores ({room.players.length}/10)</span>
          </div>
          <div className="space-y-2">
            {room.players.map(p => (
              <div key={p.uid} className="flex items-center justify-between px-4 py-3 rounded-xl bg-[#1a2a3a] border border-[#3d4a5c]">
                <div className="flex items-center gap-2">
                  {p.uid === room.hostId && <Crown size={14} className="text-yellow-400" />}
                  <span className="font-bold text-white">{p.name}</span>
                  {p.uid === user.uid && <span className="text-xs text-emerald-400">(você)</span>}
                </div>
                {isHost && p.uid !== user.uid && (
                  <button onClick={() => kickPlayer(p.uid)} className="p-1 rounded-lg hover:bg-red-500/20 text-gray-500 hover:text-red-400 transition-colors">
                    <X size={14} />
                  </button>
                )}
              </div>
        ...
        {/* Start button (host) or waiting message */}
        {isHost ? (
          <button
            onClick={handleStart}
          ...
          <div className="text-center py-4">
            ...
            <p className="text-sm text-gray-400">Aguardando o host iniciar...</p>
          </div>
        )}
      </div>
    </div>
